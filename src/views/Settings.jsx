...
export default function SettingsView({ categories, tenants, onUpdate }) {
  ...
  const handleAddCategory = async () => {
    ...
    if (error) {
      ...
      setNewCatName('');
      // Trigger App.jsx to re-fetch data so all tabs update
      onUpdate(); 
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* LEFT: CATEGORIES (Data passed from App.jsx) */}
        <GlassCard className="p-6 space-y-6">
           ...
           <div className="space-y-3">
              ...
              {categories.length === 0 && <p className="text-center text-slate-500 text-sm py-4">No categories found.</p>}
           </div>
           ...
           <div className="flex gap-2 pt-4 border-t border-white/10">
              ...
              <button onClick={handleAddCategory} className="px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm transition-colors">
                <Plus size={16} />
              </button>
           </div>
        </GlassCard>
        ...
        <GlassCard className="p-6 space-y-6 h-fit">
           ...
           <div className="space-y-3">
             ...
             {tenants.length === 0 && <p className="text-center text-slate-500 text-sm">No tenants loaded.</p>}
           </div>
        </GlassCard>
      </div>
    </div>
