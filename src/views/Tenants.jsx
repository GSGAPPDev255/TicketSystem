  ...
  const handleSave = async () => {
    ...
    if (editingTenant) {
      ...
      const { error } = await supabase
        .from('tenants')
        .update({ name: formData.name, code: formData.code, domain: formData.domain })
      ...
      const { error } = await supabase
        .from('tenants')
        .insert({ name: formData.name, code: formData.code.toUpperCase(), domain: formData.domain, status: 'Active' });
      if (error) alert(error.message);
    }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">Provision and manage school instances</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {filteredTenants.map(tenant => (
           <GlassCard key={tenant.id} className="p-6 relative group border border-white/5 hover:border-white/20 transition-all">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTenant ? "Edit Tenant" : "Provision New Tenant"}>
         <div className="space-y-4">
            <div>
               ...
               <input 
                 type="text" placeholder="e.g. Westside Academy" 
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1.5">Tenant Code (Slug)</label>
                  <input 
                    type="text" placeholder="WSA" 
                    className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-white font-mono focus:outline-none focus:border-blue-500/50"
                    value={formData.code} onChange={e => setFormData({...formData, code: e.target.value.toUpperCase()})}
                  />
               </div>
               <div>
                  <label className="block text-xs font-medium text-slate-400 mb-1.5">Domain (Optional)</label>
                  <input 
                    type="text" placeholder="westside.edu" 
                    className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500/50"
            ...
            <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
               <p className="text-xs text-blue-200 leading-relaxed">
                  <strong>Note:</strong> Creating a tenant establishes a new data silo. Users with the email domain 
                  <span className="font-mono text-white"> @{formData.domain || '...'}</span> will be automatically mapped to this tenant.
               </p>
            </div>

            <button onClick={handleSave} className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium shadow-lg shadow-blue-900/20">
