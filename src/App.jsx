...
import { 
  ...
  MoreHorizontal, Command, LogOut, Zap, Crown, ArrowRight, Loader2, Mail, Bell
} from 'lucide-react';
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-zinc-50">
            <div className="w-full max-w-md bg-white border border-zinc-200 shadow-xl rounded-3xl p-8 text-center">
                ...
                {error && <div className="mb-6 p-3 bg-red-50 text-red-600 text-xs rounded-xl flex items-center justify-center gap-2"><AlertTriangle size={14}/> {error}</div>}
...
export default function App() {
    ...
    const [routingRules, setRoutingRules] = useState([]); // Loaded from DB
    const [toast, setToast] = useState(null); // For Email Notifications
    ...
    // --- AUTH & JIT ---
    useEffect(() => {
        supabase.auth.getSession().then(({ data: { session } }) => { if (session?.user) handleUserAuth(session.user); });
        const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => { if (session?.user) handleUserAuth(session.user); });
    ...
    const handleUserAuth = async (azureUser) => {
        ...
        try {
            ...
            if (!userData) {
                ...
                if (domain.includes('kgps')) assignedSchool = 'KGPS'; // Add more mappings here
                ...
                const { data: newUser } = await supabase.from('users').insert([{
                    ...
                    access_schools: [assignedSchool], avatar_code: (azureUser.user_metadata.full_name || 'U').charAt(0).toUpperCase()
                }]).select().single();
                userData = newUser;
            }
            ...
            supabase.channel('app-db-changes')
                .on('postgres_changes', { event: '*', schema: 'public', table: 'tickets' }, () => { fetchSystemData(); showToast("Dashboard updated", "info"); })
    ...
    const handleNewTicket = async (ticket) => {
        // Find Dept Email for Notification
        const dept = departments.find(d => d.name === ticket.owner);
        const deptEmail = dept?.config?.email || "admin@system";
        ...
        if (!error) {
            showToast(`📧 Ticket Sent! Notifying ${deptEmail}...`);
            setView('admin'); // Redirect user to their dashboard to see the ticket
        }
    };

    const handleUpdateUser = async (u) => {
        await supabase.from('users').update({ role: u.role, department: u.department, school: u.school, access_schools: u.access_schools, is_super_admin: u.is_super_admin }).eq('id', u.id);
        fetchSystemData();
    ...
    const handleAddRule = async (rule) => {
        await supabase.from('routing_rules').insert([rule]);
        fetchSystemData();
    };
    ...
    const handleUpdateTicket = async (t) => {
        ...
        if (t.status === 'Resolved') {
            showToast(`📧 Closing Email sent to ${t.requester_email}`);
        } else if (t.status === 'In Progress') {
            showToast(`📧 Update sent to ${t.requester_email}`);
        }
    };
        ...
        <div className="min-h-screen flex flex-col font-sans text-zinc-900 bg-zinc-50 relative">
            ...
            {toast && (
                <div className="fixed bottom-6 right-6 z-[100] bg-zinc-900 text-white px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-enter">
                    {toast.type === 'success' ? <CheckCircle size={18} className="text-green-400"/> : <Bell size={18} className="text-blue-400"/>}
            ...
            <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-6 py-4 flex justify-between items-center shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex bg-zinc-100 p-1 rounded-lg border border-zinc-200">
                        ...
                        {/* UNLOCKED: Staff can now click "My Tickets" */}
                ...
                {view === 'chat' ? (
                    ...
                    <AdminDashboard 
                        ...
                        tickets={tickets.filter(t => {
                            ...
                            if (currentUser.role === 'Staff') return t.requester_email === currentUser.email; // Staff only see own
                            // Delegated Admin Logic (School + Dept)
    ...
    const handleSend = (text) => {
        ...
        setTimeout(() => {
            if (step === 'ISSUE') {
                ...
                setMessages(prev => [...prev, { id: Date.now(), sender: 'bot', text: "Ticket created! You can track it in your Dashboard." }]);
                setStep('DONE');
            }
        }, 500);
        setInput('');
...
// 2. ADMIN DASHBOARD (With Routing & Permission Logic)
    ...
    return (
        <div className="h-full flex flex-col bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden relative bg-zinc-50/30">
                ...
                {activeTab === 'directory' && <UserDirectory users={users} currentUser={currentUser} schools={schools} departments={departments} onUpdateUser={onUpdateUser} />}
...
// 3. TICKET DETAIL (Resolving triggers Email Simulation)
            ...
            {showResolveModal && (
                <div className="absolute inset-0 bg-white/90 backdrop-blur-sm z-50 flex items-center justify-center p-8">
                    <div className="w-full max-w-md bg-white border border-zinc-200 shadow-2xl rounded-2xl p-6">
                        <h3 className="font-bold text-lg mb-4">Resolve Ticket?</h3>
                        <p className="text-xs text-zinc-500 mb-4">This will send a closing email notification to <strong>{ticket.requester_email}</strong>.</p>
                        <div className="flex gap-3"><button onClick={() => setShowResolveModal(false)} className="flex-1 px-4 py-2 rounded-lg border">Cancel</button><button onClick={() => { onUpdateTicket({ ...ticket, status: 'Resolved' }); sendMessage("Ticket Resolved", "SYSTEM"); setShowResolveModal(false); }} className="flex-1 px-4 py-2 rounded-lg bg-green-600 text-white font-bold">Confirm Resolution</button></div>
                    </div>
                </div>
            )}
